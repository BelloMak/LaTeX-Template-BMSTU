...
- [My BMSTU LaTeX class](#my-bmstu-latex-class)
  ...
  - [Рекомендации по написанию работы для успешной проверки программой TestVKR](#рекомендации-по-написанию-работы-для-успешной-проверки-программой-testvkr)
...
## Рекомендации по написанию работы для успешной проверки программой TestVKR
Далее приведены рекомендации, как избежать некоторых ошибок при проверке
программой TestVKR:
- Не стоит писать формулы с нового абзаца, то есть добавлять их в текст так:
  ```
  Рассчитаем коэффициент по формуле:
  \begin{equation}
    R_\varphi = \frac{1}{\frac{K_\varphi}{mgh} - 1}
  \end{equation}
  ```
  а не так:
  ```
  ...
  \end{equation}
  ```
- Не стоит добавлять несколько источников цитирования с помощью одной команды
  `\cite`, то есть добавлять их в текст так:
  ```
  В статье~\cite{article:some_ref}~\cite{article:some_another_ref} ...
  ```
  а не так:
  ```
  В статье~\cite{article:some_ref, article:some_another_ref} ...
  ```
- Стоит избегать вставки изображений с высоким разрешением, так как, видимо,
  программа высчитывает размер картинок исходя из их разрешения, что иногда
  приводит к ошибкам
- Стоит избегать вставки изображений из pdf, особенно чертежей, так как на них
  часто есть текст, который парсит TestVKR и ругается на него
- Часто программа показывает предупреждение вида: "Документ
  содержит страницы, размеры правого поля на которых не соответствуют
  требованиям, или нет знаков препинаний в конце коротких строк, или не
  используется выравнивание текста по ширине:". Пока установить причину его
  появления не получилось